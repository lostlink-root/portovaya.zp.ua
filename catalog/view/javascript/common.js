...
$(document).ajaxStop(function() {
	...
	if (!isEmpty($('#product'))) {
		  $('.product-details .option-container').addClass('has-option');
	}
});
$(document).ready(function() {
	...
	//let a = document.getElementById("payment_address_city")
	//alert();
		//document.getElementById("payment_address_city") &&
		//var rr = $('#payment_address_city');
		//$('payment_address_city').setAttribute("name","city");
...
(function($) {
	...
	$(document).mouseup(function (e){ // событие клика по веб-документу
		...
		var closeIcon = $(".cart-dropdown-menu-close");
		var closeIconPlus = $(".ion-android-close");
		if ((!div.is(e.target) // если клик был не по нашему блоку
		    && div.has(e.target).length === 0 ) || closeIcon.is(e.target) || closeIconPlus.is(e.target)) { // и не по его дочерним элементам
