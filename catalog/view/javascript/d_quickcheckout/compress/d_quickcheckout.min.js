...
(function (root, factory) {
    ...
    'use strict';
    var _delayedTriggers = [], nestedChanges;
    Backbone.NestedModel = Backbone.Model.extend({
        get: function (attrStrOrPath) {
            ...
            var newAttrs = Backbone.NestedModel.deepClone(this.attributes), attrPath, unsetObj, validated;
            if (_.isString(key)) {
            ...
            opts = opts || {};
            var attrPath = Backbone.NestedModel.attrPath(attrStr), aryPath = _.initial(attrPath),
                val = this.get(aryPath), i = _.last(attrPath);
            if (!_.isArray(val)) {
                throw new Error("remove() must be called on a nested array");
            }
            var trigger = !opts.silent && (val.length >= i + 1), oldEl = val[i];
            val.splice(i, 1);
            opts.silent = true;
        ...
        attrPath: function (attrStrOrPath) {
            ...
            var attrPath = Backbone.NestedModel.attrPath(attrStrOrPath), result;
...
qc.View = Backbone.View.extend({
    ...
    events: {'focus input': "updateFocus"},
...
qc.LoginView = qc.View.extend({
    ...
    events: {'change input[name=account]': 'updateAccount', 'click button#button_login': 'loginAccount'},
    template: '',
    render: function () {
...
qc.FieldView = qc.View.extend({
    initialize: function (e) {
        ...
        var that = this;
        $('.date', this.el).datetimepicker({pickTime: false, dateFormat: "mm/DD/YYYY",})
        $('.time', this.el).datetimepicker({pickDate: false, dateFormat: "mm/DD/YYYY",})
        $('.datetime', this.el).datetimepicker({pickDate: true, pickTime: true, dateFormat: "mm/DD/YYYY",})
        ...
        $(this.el).validate({
            submitHandler: function (form) {
                ...
                $(element.form).find("#" + element.id.replace(/\./g, '\\\.') + "_input").addClass("has-error");
            }, unhighlight: function (element, errorClass, validClass) {
                $(element.form).find("#" + element.id.replace(/\./g, '\\\.') + "_input").removeClass("has-error");
            }, errorClass: "text-danger", errorElement: "div"
        });
        ...
        if ($('.telephone-validation').length !== 0) {
            if ($.trim($(e.currentTarget).val())) {
                if ($(e.currentTarget).intlTelInput("isValidNumber")) {
                    ...
                    $(e.currentTarget).val('');
                    $(e.currentTarget).parents('.text-input').removeClass("has-error").find('.text-danger').remove();
...
qc.PaymentAddress = qc.Model.extend({
    defaults: {'payment_address': '', 'config': '', 'account': '', 'addresses': '',}, initialize: function () {
        ...
        if (typeof (this.get('payment_address.' + options.key)) !== 'undefined' && this.get('payment_address.' + options.key).length > 0 && typeof (attrs.payment_address[options.key]) !== 'undefined' && attrs.payment_address[options.key].length == 0) {
            ...
            errors.push({
                ...
                key: options.key,
                value: this.get('payment_address.' + options.key)
            });
...
qc.PaymentAddressView = qc.View.extend({
    ...
    update: function (data) {
        ...
        if (data.payment_address) {
            ...
            _.each(data.payment_address, function (element, index) {
                ...
                that.model.set('payment_address.' + index, element, {
                    validate: true,
                    key: index,
                    value: element,
                    error: that.model.handleError()
                });
            });
        }
    ...
    render: function () {
        ...
        $('#' + this.focusedElementId).focus();
        // $("#payment_address_city").attr("name", "city");
        initAutocomplete();
        checkAddressVisibility();
    },
    zoneDelay: function () {
...
qc.ShippingAddress = qc.Model.extend({
    defaults: {'shipping_address': '', 'config': '', 'account': '', 'addresses': '', 'show_shipping_address': '',},
    initialize: function () {
    ...
    update: function (data) {
        ...
        if (data.shipping_address) {
            ...
            _.each(data.shipping_address, function (element, index) {
                if (index.indexOf('custom_field.') != -1) {
                    qc.shippingAddress.attributes.shipping_address[index] = element;
                }
                that.model.set('shipping_address.' + index, element, {
                    validate: true,
                    key: index,
                    value: element,
                    error: that.model.handleError()
...
qc.ShippingMethodView = qc.View.extend({
    ...
    events: {'change input[type=radio]': 'updateShippingMethod', 'change select': 'updateShippingMethod',},
    template: '',
...
qc.PaymentMethod = qc.Model.extend({
    defaults: '', initialize: function () {
        ...
        var that = this;
        $.post('index.php?route=extension/d_quickcheckout/payment_method/update', json, function (data) {
            that.updateForm(data);
        }, 'json').error();
    },
});
qc.PaymentMethodView = qc.View.extend({
    ...
    events: {'change input[type=radio]': 'updatePaymentMethod', 'change select': 'updatePaymentMethod',},
    template: '',
...
qc.PaymentView = qc.View.extend({
    initialize: function (e) {
        ...
        this.model.on("change", this.render, this);
        this.render();
    }, template: '', paymentConfirm: function () {
...
qc.Confirm = qc.Model.extend({
    defaults: {'confirm': '', 'config': '', 'account': '', 'data': ''},
    initialize: function () {
...
qc.ConfirmView = qc.View.extend({
    initialize: function (e) {
        ...
        qc.event.bind("paymentConfirm", this.paymentConfirm, this);
        this.render();
    }, events: {'click button#qc_confirm_order': 'confirm',}, template: '', forceConfirm: function () {
        var that = this;
